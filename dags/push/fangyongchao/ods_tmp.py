...
@dag(schedule_interval='0 */2 * * *',
     start_date=pendulum.datetime(2025, 6, 5), end_date=pendulum.datetime(2025, 6, 9), catchup=True, 
     default_args=default_args, tags=['ods', 'kuaishou'])
def ods_tmp():
    ...
    def get_open_id():
        ...
        where status = 1
            ...
            and open_id = 'f18d5ec29982990414bf5724d18f0767'
    ...
    def summary(num, **kwargs):
        from airflow.models import Variable
        total = sum(num)
        begin_time = kwargs['data_interval_start']
        end_time = kwargs['data_interval_end']
        begin_time = begin_time.in_tz('Asia/Shanghai').format('YYYY-MM-DD HH:mm:ss')
        end_time = end_time.in_tz('Asia/Shanghai').format('YYYY-MM-DD HH:mm:ss')
        from airflow.providers.common.sql.operators.sql import SQLExecuteQueryOperator
        sql = f'''
    ...
    path = fetch_write_data.expand(tokens=tokens)
