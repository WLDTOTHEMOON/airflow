...
class SlicePerformance(FeishuNotificationDAG):
    def __init__(self):
        super().__init__(
            dag_id='push_slice',
    ...
    def fetch_data_logic(self, date_interval: Dict[str, Any]) -> Dict[str, Any]:
        ...
        all_df = pd.read_sql(sql_dict['all_sql'], self.conn)
        # 日切片整体业绩
        ...
        # 商品数据
        accum_rank_items = pd.read_sql(sql_dict['accum_rank_items'], self.conn)
        ...
        yes_rank_items = pd.read_sql(sql_dict['yes_rank_items'], self.conn)
    ...
    def prepare_card_logic(self, data: Dict[str, Any], date_interval: dict = None) -> Dict[str, Any]:
        yes_group_slice_tol = []
        for i in range(data['df']['yes_each_tol_df'].shape[0]):
            yes_group_slice_tol.append({
                'belong': data['df']['yes_each_tol_df'].slice_belong.iloc[i],
                'origin_gmv': str(round(data['df']['yes_each_tol_df'].origin_gmv.iloc[i], 2)),
                'final_gmv': str(round(data['df']['yes_each_tol_df'].final_gmv.iloc[i], 2)),
                'origin_order_number': str(int(data['df']['yes_each_tol_df'].origin_order_number.iloc[i])),
                'tol_income': str(round(data['df']['yes_each_tol_df'].tol_income.iloc[i], 2))
            })
        group_slice_tol = []
        for i in range(data['df']['each_tol_df'].shape[0]):
            group_slice_tol.append({
                'belong': data['df']['each_tol_df'].slice_belong.iloc[i],
                'origin_gmv': str(round(data['df']['each_tol_df'].origin_gmv.iloc[i], 2)),
                'final_gmv': str(round(data['df']['each_tol_df'].final_gmv.iloc[i], 2)),
                'origin_order_number': str(int(data['df']['each_tol_df'].origin_order_number.iloc[i])),
                'tol_income': str(round(data['df']['each_tol_df'].tol_income.iloc[i], 2))
            })
        return {
            'time_scope': date_interval['month_start_time'] + ' to ' + date_interval['yes_time'],
            'slice_yes_date': date_interval['yes_ds'],
    ...
    def write_to_sheet_logic(self, data: Dict[str, Any], date_interval: dict = None) -> Dict[str, Any]:
        all_df = data['df']['all_sql']
        ...
        workbook_name = f"切片卖货数据_{date_interval['month_start_time']}_{date_interval['yes_time']}_{date_interval['now_time']}"
        ...
        slice_sheet_token = self.feishu_sheet.create_sheet(spreadsheet_token, '切片卖货数据')
        other2_rank_sheet_token = self.feishu_sheet.create_sheet(spreadsheet_token, '电商MCN(乐总)当月商品排名')
        other1_rank_sheet_token = self.feishu_sheet.create_sheet(spreadsheet_token, '小仙女当月商品排名')
        le_rank_sheet_token = self.feishu_sheet.create_sheet(spreadsheet_token, '乐总当月商品排名')
        yes_other2_rank_sheet_token = self.feishu_sheet.create_sheet(spreadsheet_token, '电商MCN(乐总)昨日商品排名')
        yes_other1_rank_sheet_token = self.feishu_sheet.create_sheet(spreadsheet_token, '小仙女昨日商品排名')
        yes_le_rank_sheet_token = self.feishu_sheet.create_sheet(spreadsheet_token, '乐总昨日商品排名')
        total_sheet_token = self.feishu_sheet.create_sheet(spreadsheet_token, '月汇总数据', index=0)
        self.feishu_sheet.write_dat_replace(belong_sum_df, spreadsheet_token, slice_sheet_token, to_char=False)
        self.feishu_sheet.write_dat_replace(data['df']['accum_other2_rank_items'], spreadsheet_token,
                                            other2_rank_sheet_token, to_char=False)
        self.feishu_sheet.write_dat_replace(data['df']['accum_other1_rank_items'], spreadsheet_token,
                                            other1_rank_sheet_token, to_char=False)
        self.feishu_sheet.write_dat_replace(data['df']['accum_le_rank_items'], spreadsheet_token,
                                            le_rank_sheet_token, to_char=False)
        self.feishu_sheet.write_dat_replace(data['df']['yes_other2_rank_items'], spreadsheet_token,
                                            yes_other2_rank_sheet_token, to_char=False)
        self.feishu_sheet.write_dat_replace(data['df']['yes_other1_rank_items'], spreadsheet_token,
                                            yes_other1_rank_sheet_token, to_char=False)
        self.feishu_sheet.write_dat_replace(data['df']['yes_le_rank_items'], spreadsheet_token,
                                            yes_le_rank_sheet_token, to_char=False)
        self.feishu_sheet.write_dat_replace(data['df']['df_total'], spreadsheet_token, total_sheet_token, to_char=False)
        return {
            'sheet_params': {
...
push_slice_performance = SlicePerformance().create_dag()
