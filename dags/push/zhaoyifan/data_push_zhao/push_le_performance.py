...
class LePerformance(BaseDag):
    def __init__(self):
        super().__init__(
            dag_id='push_le_performance',
            robot_url=Variable.get('RYOMA'),
            tags=['push', 'le_performance'],
            schedule='0 5 * * *'
        )
        self.card_id = 'AAq4gfjP7EbHF'
    ...
    def write_to_sheet_logic(self, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sheet_title = f"乐总业绩数据_{data_dict['date_interval']['month_start_time']}_{data_dict['date_interval']['yes_time']}_{data_dict['date_interval']['now_time']}"
