       ...
       '''
        ...
        anchor_sql = f'''
            select 
                ...
                ,sum(final_gmv) final_gmv
                ,sum(final_gmv) / sum(origin_gmv) final_rate
                ,sum(commission_income) commission_income
            from (            
                select 
                    account_id
                    ,case 
            ...
            left join (
                select 
                    *
                from tmp.tmp_ks_ec_corrected tkec 
            ) cor on gmv.account_id = cor.origin_account_id 
                and gmv.order_date between cor.start_date and cor.end_date
            group by 
                1,2
            order by 
                order_date
        ...
        anchor_target_sql = f'''
            ...
            left join (
              ...
              group by 1
            ) target on gmv.anchor_name = target.anchor
            group by 1,2
